...
export interface AuroraBlurProps {
  ...
  opacity?: number;
  /**
   * Lantern Fire mod: optional hex color (e.g. `"#2a1a0a"`) blended in at the
   * top of the canvas, anchoring the "atmosphere" above the aurora ribbons on
   * a brand color instead of falling back to pure black. Default `"#000000"`
   * preserves the original aurora-borealis-style night-sky behaviour.
   */
  topColor?: string;
  /**
   * Lantern Fire mod: optional hex color blended in at the bottom of the
   * canvas, mirroring `topColor`. Locks the very bottom edge to the exact
   * color passed in (typically the page `--background`) so the bottom of
   * the canvas seamlessly meets the section below instead of being washed
   * out by additive sky-layer blow-out. Default `"#000000"` preserves the
   * original behaviour.
   */
  bottomColor?: string;
}
...
void main(){
  ...
  // Lantern Fire mod: top-anchor brand color is applied AFTER saturation /
  // brightness as a mix() rather than as an additive layer before them. This
  // lets the very top of the canvas lock to the exact u_topColor passed in,
  // rather than getting subtracted from by negative aurora contributions at
  // the top (where verticalFade pushes the noise term below zero) and then
  // dimmed by the brightness multiplier. The step() gate keeps the default
  // (u_topColor = vec3(0)) a no-op so other AuroraBlur consumers are
  // unaffected.
  float topMixWeight = smoothstep(0.4, 1.0, uv.y) * step(0.001, length(u_topColor));
  c = mix(c, u_topColor, topMixWeight);

  // Lantern Fire mod: bottom-anchor brand color mirrors u_topColor, locking
  // the very bottom of the canvas to u_bottomColor (typically the page
  // --background) so the aurora's lower sky-layer additive stack doesn't
  // blow out to near-white. Range (0.0, 0.65) overlaps slightly with the
  // top mix's lower bound (0.4) for a longer, less abrupt fade up from the
  // bottom; the overlap zone is small because both weights are near zero
  // there. Gated by step() so the default is a no-op.
  float bottomMixWeight = (1.0 - smoothstep(0.0, 0.65, uv.y)) * step(0.001, length(u_bottomColor));
  c = mix(c, u_bottomColor, bottomMixWeight);
...
const Scene: React.FC<SceneProps> = ({
  ...
  const uniforms = useMemo(
    () => ({
      ...
      u_opacity: { value: 1 },
      u_topColor: { value: new THREE.Vector3(0, 0, 0) },
      u_bottomColor: { value: new THREE.Vector3(0, 0, 0) },
    }),
    [],
  );
  ...
  useFrame((state) => {
    ...
    material.uniforms.u_topColor.value.set(...hexToVec3(topColor || "#000000"));
    material.uniforms.u_bottomColor.value.set(
      ...hexToVec3(bottomColor || "#000000"),
    );
  });
